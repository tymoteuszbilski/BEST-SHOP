...
function count() {
  ...
  let amounts: number[] = new Array(unique.length);
  amounts.fill(0);

  for (let i = 0; i < unique.length; i++) {
    for (let j = 0; j < all.length; j++) {
      if (
        unique[i].id === all[j].id &&
...
function getUniqueProducts() {
  ...
  let uniqueStrings = productsStrings.filter(
...
function createTableRow(product: Product, amount: number) {
